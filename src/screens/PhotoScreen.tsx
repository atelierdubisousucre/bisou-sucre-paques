...
export default function PhotoScreen() {
  ...
  // ── WEB : compose avec canvas, puis save/share adapté au navigateur ───────
  const handleWeb = useCallback(async (action: 'save'|'share') => {
    if (!photoUri) return;
    setCompositing(true);
    try {
      // 1. Composer photo + cadre via canvas
      ...
      // 2. Détecter iOS Safari (navigator.share avec files est la seule option)
      const isIosSafari = /iPad|iPhone|iPod/.test(navigator.userAgent);
      const canShare    = !!(navigator as any).share && !!(navigator as any).canShare;

      if (isIosSafari || action === 'share') {
        // iOS Safari + partage : utiliser Web Share API
        if (canShare) {
          const file = new File([blob], 'joyeuses-paques.jpg', { type:'image/jpeg' });
          if ((navigator as any).canShare({ files: [file] })) {
            await (navigator as any).share({
              files:  [file],
              title:  'Joyeuses Paques !',
              text:   'Ma photo de Paques avec L\'Atelier du Bisou Sucre',
            });
            return; // succès
          }
        }
        // Fallback iOS : ouvrir dans nouvel onglet (l'utilisateur peut "Enregistrer l'image")
        const url  = URL.createObjectURL(blob);
        const win  = window.open(url, '_blank');
        if (!win) {
          // Si le popup est bloqué : afficher l'image inline
          ...
          img.title = 'Appuyez longuement pour enregistrer';
          img.onclick = () => document.body.removeChild(img);
          document.body.appendChild(img);
          Alert.alert(
            'Enregistrer la photo',
            'Appuyez longuement sur la photo puis selectionnez "Enregistrer l\'image".'
          );
        }
        setTimeout(() => URL.revokeObjectURL(url), 10000);
      } else {
        // Chrome / Android / Desktop : téléchargement direct
        const url = URL.createObjectURL(blob);
...
function go(){
  ...
  window.ReactNativeWebView.postMessage(
    JSON.stringify({ok:true,data:c.toDataURL('image/jpeg',0.92)})
  );
}
...
f.src='${FRAME_B64}';
  ...
  // ── Résultat WebView (mobile) ─────────────────────────────────────────────
  const onWebViewMsg = useCallback(async (event: any) => {
    try {
      const msg    = JSON.parse(event.nativeEvent.data);
      const action = pendingAction;
      setWebViewHTML(null); setPendingAction(null);
    ...
    try {
      ...
      if (!perm?.granted) {
        Alert.alert('Permission requise',
          IS_IOS
            ? 'Reglages → Expo Go → Photos → Autoriser l\'acces complet'
      ...
      if (IS_IOS) {
        // iOS : createAssetAsync + album dédié
        const asset = await MediaLibrary.createAssetAsync(uri);
        try {
          let album = await MediaLibrary.getAlbumAsync('Joyeuses Paques');
          if (album) await MediaLibrary.addAssetsToAlbumAsync([asset], album, false);
          else await MediaLibrary.createAlbumAsync('Joyeuses Paques', asset, false);
        } catch { /* photo sauvegardée même si album échoue */ }
        Alert.alert('Photo sauvegardee !',
          'Retrouvez-la dans l\'album "Joyeuses Paques" de votre galerie 🐣');
      } else {
      ...
      Alert.alert('Sauvegarde impossible',
        'Utilisez "Partager" → "Enregistrer dans Photos".',
    ...
    try {
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          mimeType: 'image/jpeg',
          dialogTitle: 'Votre photo de Paques',
          UTI: 'public.jpeg',
        });
      }
  ...
  const handleSave  = useCallback(() =>
    IS_WEB ? handleWeb('save')  : handleMobile('save'),  [handleWeb, handleMobile]);
  const handleShare = useCallback(() =>
    IS_WEB ? handleWeb('share') : handleMobile('share'), [handleWeb, handleMobile]);
  ...
  // ── Permission caméra ──────────────────────────────────────────────────────
  if (!camPerm) return (
    <View style={s.ctr}><ActivityIndicator size="large" color="#FF6B9D"/></View>
  );
  if (!camPerm.granted) {
    return (
      ...
      <View style={{ flex:1, backgroundColor:'#0a0a0a' }}>
        ...
        <LinearGradient colors={['transparent','rgba(0,0,0,0.92)']}
          ...
          <View style={s.prevBtns}>
            <TouchableOpacity style={[s.pBtn,s.btnRetake]}
              onPress={retake} disabled={compositing}>
              <Text style={s.pBtnIcon}>🔄</Text>
              <Text style={s.pBtnTxt}>Reprendre</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[s.pBtn,s.btnSave]}
              onPress={handleSave} disabled={compositing}>
              {compositing
                ? <ActivityIndicator color="#fff" size="small"/>
                : <Text style={s.pBtnIcon}>💾</Text>}
              <Text style={s.pBtnTxt}>{IS_WEB?'Sauvegarder':'Sauvegarder'}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[s.pBtn,s.btnShare]}
              onPress={handleShare} disabled={compositing}>
              {compositing
                ? <ActivityIndicator color="#fff" size="small"/>
                : <Text style={s.pBtnIcon}>📤</Text>}
