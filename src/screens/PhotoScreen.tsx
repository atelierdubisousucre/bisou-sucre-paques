...
// Import conditionnel — FileSystem et WebView ne sont pas disponibles sur web
let FileSystem: any = null;
let WebView: any = null;
let FRAME_B64: string = '';

if (Platform.OS !== 'web') {
  FileSystem = require('expo-file-system');
  WebView    = require('react-native-webview').WebView;
  FRAME_B64  = require('../utils/frameData').FRAME_B64;
}
...
// HTML Canvas pour la composition (mobile uniquement)
const makeCompositorHTML = (photoB64: string) => `<!DOCTYPE html><html>
...
(function(){
  ...
  function draw(){
    ...
    window.ReactNativeWebView.postMessage(JSON.stringify({
      ok:true, data:c.toDataURL('image/jpeg',0.95)
    }));
  }
  function err(e){
    window.ReactNativeWebView.postMessage(JSON.stringify({ok:false,msg:''+e.type}));
  }
...
export default function PhotoScreen() {
  ...
  const webCanvasRef = useRef<HTMLCanvasElement|null>(null); // pour web
  ...
  useEffect(() => {
    (async () => {
      ...
      if (Platform.OS !== 'web' && !mediaPerm?.granted) await reqMediaPerm();
    })();
  }, []);
  ...
  // ── Composition WEB : canvas HTML5 natif ──────────────────────────────────
  const composeWeb = useCallback(async (action: 'save'|'share') => {
    ...
    try {
      ...
      await new Promise<void>((res, rej) => {
        ...
        photo.src = photoUri;
        // Charger le cadre depuis les assets publics
        frame.src = `${window.location.origin}${window.location.pathname.replace(/\/$/, '')}/assets/cadre.png`;
      ...
      if (action === 'share' && navigator.share) {
        ...
        await navigator.share({ files: [file], title: 'Photo de Pâques 🐣' });
      } else {
      ...
      Alert.alert('Erreur', 'Composition échouée. Réessayez.');
    } finally {
      setCompositing(false);
    }
  }, [photoUri]);

  // ── Composition MOBILE : WebView canvas ───────────────────────────────────
  const startCompositing = useCallback(async (action: 'save'|'share') => {
    if (Platform.OS === 'web') { await composeWeb(action); return; }
    ...
    try {
      ...
      setCompositorHTML(makeCompositorHTML(photoB64));
    } catch (e) {
  ...
  const onWebViewMsg = useCallback(async (event: any) => {
    try {
      ...
      if (!msg.ok) {
        setCompositing(false);
        Alert.alert('⚠️ Cadre indisponible',
  ...
  const doSave = async (uri: string) => {
    try {
      ...
      Alert.alert('❌ Erreur', 'Sauvegarde impossible. Essayez de partager.', [
        { text: 'OK' },
  ...
  // ── Permission ─────────────────────────────────────────────────────────────
  if (!camPerm) {
    return <View style={s.ctr}><ActivityIndicator size="large" color="#FF6B9D"/></View>;
  }
  if (!camPerm.granted) {
    return (
      <LinearGradient colors={['#FFF0F5','#FFE4F0']} style={s.ctr}>
        ...
        {Platform.OS !== 'web' && (
          <TouchableOpacity
            style={[s.permBtn, { marginTop: 10, backgroundColor: '#888' }]}
  ...
  if (phase === 'preview' && photoUri) {
    return (
      <View style={{ flex: 1, backgroundColor: '#0a0a0a' }}>
        {/* WebView cachée — mobile uniquement */}
        {Platform.OS !== 'web' && compositorHTML && WebView && (
          <WebView
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#000' }}>
      ...
      <View pointerEvents="none"
        style={{ position: 'absolute', top: frameTop + 14,
...
const s = StyleSheet.create({
  ...
  permBtn: { backgroundColor: '#FF6B9D', paddingHorizontal: 32,
    paddingVertical: 14, borderRadius: 30, elevation: 6, marginBottom: 4 },
  permBtnTxt: { color: '#fff', fontWeight: 'bold', fontSize: 16,
    textAlign: 'center' },
  ...
  prevBtns: { flexDirection: 'row', justifyContent: 'space-around',
    alignItems: 'center' },
