/**
 * TaquinScreen v8
 * - 6 sauvegardes indépendantes (1 par combinaison image×difficulté)
 * - Restauration au moment du lancement du jeu (pas à l'ouverture du menu)
 * - Swipe haut/bas/gauche/droite avec PanResponder
 */
...
const BOARD_BASE    = SW - BOARD_PADDING * 2;
const SWIPE_MIN     = 10;
...
// Clé AsyncStorage unique par combinaison image×difficulté
...
const TaquinTile = React.memo(function TaquinTile({
  ...
  imageSource, onSwipe, isEmpty, scaleAnim,
}: TileProps) {
...
export default function TaquinScreen() {
  ...
  const [hasSave,       setHasSave]       = useState(false); // indicateur de sauvegarde
  ...
  // ── Vérifier si une sauvegarde existe pour la combo sélectionnée ──────────
  const checkSave = useCallback(async (imgIdx: ImageIndex, diffIdx: DiffIndex) => {
    try {
      const key = saveKey(IMAGES[imgIdx].id, DIFFICULTIES[diffIdx].id);
      const raw = await AsyncStorage.getItem(key);
      if (!raw) { setHasSave(false); return; }
      const data = JSON.parse(raw);
      const grid = DIFFICULTIES[diffIdx].grid;
      const valid = Array.isArray(data.tiles) &&
        data.tiles.length === grid*grid &&
        !isSolved(data.tiles);
      setHasSave(valid);
    } catch { setHasSave(false); }
  }, []);

  // Vérifier au montage et à chaque changement de sélection
  useEffect(() => { checkSave(selectedImage, selectedDiff); }, [selectedImage, selectedDiff]);

  // ── Persist ────────────────────────────────────────────────────────────────
  const persist = useCallback(async (
    t: number[], imgIdx: ImageIndex, diffIdx: DiffIndex
  ) => {
    try {
      const key = saveKey(IMAGES[imgIdx].id, DIFFICULTIES[diffIdx].id);
  ...
  // ── Démarrer — avec vérification de sauvegarde ────────────────────────────
  const handleStart = useCallback(async () => {
    const imgIdx  = selectedImage;
    const diffIdx = selectedDiff;
    const diff    = DIFFICULTIES[diffIdx];

    // Vérifier s'il existe une sauvegarde pour cette combo
    try {
      ...
      if (raw) {
        const data = JSON.parse(raw);
        const valid = Array.isArray(data.tiles) &&
          data.tiles.length === diff.grid*diff.grid &&
          !isSolved(data.tiles);
        ...
        if (valid) {
          ...
          Alert.alert(
            '💾 Partie en cours',
            `Vous avez une partie ${IMAGES[imgIdx].label} en mode ${diff.label} non terminée.\nQue voulez-vous faire ?`,
            [
              {
                text: '▶️ Reprendre',
                onPress: () => launchGame(imgIdx, diffIdx, data.tiles),
              },
              {
    ...
    // Pas de sauvegarde → nouvelle partie directement
    const sh = shuffleFromSolved(diff.grid, diff.moves);
    launchGame(imgIdx, diffIdx, sh);
  }, [selectedImage, selectedDiff]);

  // ── Lancer le jeu avec des tuiles données ─────────────────────────────────
  const launchGame = (imgIdx: ImageIndex, diffIdx: DiffIndex, t: number[]) => {
    ...
    // Sauvegarder l'état initial si c'est une nouvelle partie
    if (!isSolved(t)) {
      persist(t, imgIdx, diffIdx);
    }
  };
  ...
  const handleSwipe = useCallback((pos:number, dir:'up'|'down'|'left'|'right') => {
    ...
    const valid =
      (dir==='right'&&er===r&&ec===c+1)||
      (dir==='left' &&er===r&&ec===c-1)||
      (dir==='down' &&ec===c&&er===r+1)||
      (dir==='up'   &&ec===c&&er===r-1);
    ...
    Animated.sequence([
      Animated.timing(anim,{toValue:0.88,duration:50,useNativeDriver:true}),
      Animated.spring(anim,{toValue:1,useNativeDriver:true,tension:400,friction:8}),
    ]).start();
  ...
  // ── Sélecteurs ────────────────────────────────────────────────────────────
  const selectImg = (i:ImageIndex) => {
    imgRef.current = i; setSelectedImage(i);
  };
  ...
  const leaveGame = useCallback(()=>{
    Alert.alert('Quitter','Progression sauvegardée.',[
      ...
      {text:'Quitter',onPress:()=>setPhase('select')},
    ]);
  },[]);
    ...
    return (
      <LinearGradient colors={['#FFF0F5','#FFE4F0','#FECDE0']} style={{flex:1}}>
        <SafeAreaView style={{flex:1}}>
          <ScrollView contentContainerStyle={s.sel} showsVerticalScrollIndicator={false}>
            ...
            <View style={s.imgRow}>
              {IMAGES.map((img,idx)=>(
                <TouchableOpacity key={img.id}
                  ...
                  <Text style={[s.imgLabel,selectedImage===idx&&s.imgLabelSel]}>
                    {img.label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <View style={s.diffRow}>
              {DIFFICULTIES.map((d,idx)=>(
                <TouchableOpacity key={d.id}
                  ...
                  <Text style={[s.diffLabel,selectedDiff===idx&&s.diffLabelSel]}>
                    {d.label}
                  </Text>
                  <Text style={s.diffSub}>
                    {d.grid}×{d.grid} · {d.grid*d.grid-1} pièces
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={s.helpBox}>
              ...
              <Text style={s.helpTxt}>
                Repérez la <Text style={{fontWeight:'800'}}>case sombre</Text>{' '}
                (case vide).{'\n'}
                Swipez une pièce adjacente{' '}
                <Text style={{fontWeight:'800'}}>vers cette case</Text>.{'\n'}
                Directions : ← → ↑ ↓ (pas de diagonal)
              </Text>
            </View>

            {/* Bouton avec indicateur de sauvegarde */}
  ...
  return (
    <LinearGradient colors={['#FFF0F5','#FFE4F0']} style={{flex:1}}>
      <SafeAreaView style={{flex:1}}>
          ...
          <View style={{width:boardSize,height:boardSize,backgroundColor:'#C2185B',
            ...
            {tiles.map((tileValue,position)=>(
              <TaquinTile key={String(position)}
                ...
                tileSize={tileSize} boardSize={boardSize}
        ...
        {complete&&(
          <View style={s.victory}>
            ...
            <View style={{flexDirection:'row',gap:12}}>
              ...
              <TouchableOpacity style={s.vBtn} onPress={()=>setPhase('select')}>
        ...
        {!complete&&(
          <Text style={s.hint}>← → ↑ ↓  Swipez une pièce vers la case sombre</Text>
        )}
...
const s = StyleSheet.create({
  sel:{padding:BOARD_PADDING,paddingBottom:40},
  ...
  imgCard:{width:'48%',borderRadius:18,overflow:'hidden',borderWidth:3,
    borderColor:'#E0E0E0',backgroundColor:'#fff',elevation:4},
  imgCardSel:{borderColor:'#FF6B9D'},
  imgPreviewWrap:{height:130,backgroundColor:'#F5F5F5'},
  imgPreviewImg:{width:'100%',height:'100%'},
  imgPreviewOv:{...StyleSheet.absoluteFillObject,justifyContent:'center',
    alignItems:'center',backgroundColor:'rgba(255,240,245,0.5)'},
  qmark:{fontSize:52,fontWeight:'900',color:'#C2185B'},
  imgLabel:{textAlign:'center',paddingVertical:10,fontSize:13,
    fontWeight:'700',color:'#666'},
  imgLabelSel:{color:'#C2185B'},
  diffRow:{flexDirection:'row',justifyContent:'space-between',marginBottom:20},
  diffCard:{flex:1,marginHorizontal:4,padding:12,borderRadius:16,borderWidth:2,
    borderColor:'#E0E0E0',alignItems:'center',backgroundColor:'#fff',elevation:3},
  ...
  helpBox:{backgroundColor:'rgba(194,24,91,0.07)',borderRadius:14,
    padding:14,marginBottom:20,borderLeftWidth:4,borderLeftColor:'#FF6B9D'},
  ...
  hdr:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',
    ...
    borderBottomColor:'rgba(194,24,91,0.1)'},
  hdrBtn:{padding:6},
  ...
  hint:{textAlign:'center',color:'#AD1457',fontSize:13,
    paddingHorizontal:30,paddingVertical:8,opacity:0.85},
  modalBg:{flex:1,backgroundColor:'rgba(0,0,0,0.88)',justifyContent:'center',
    alignItems:'center',padding:20},
  modalBox:{backgroundColor:'#fff',borderRadius:24,overflow:'hidden',
    width:'100%',maxWidth:400,alignItems:'center',paddingBottom:20},
  modalTitle:{fontSize:17,fontWeight:'800',color:'#C2185B',paddingVertical:16,
    textAlign:'center',paddingHorizontal:16},
  mClose:{flex:1,paddingVertical:12,borderRadius:20,
    backgroundColor:'#EEE',alignItems:'center'},
  mNew:{flex:1,paddingVertical:12,borderRadius:20,
    backgroundColor:'#FF6B9D',alignItems:'center'},
});
